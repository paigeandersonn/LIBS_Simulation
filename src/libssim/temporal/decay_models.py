...
=============================
Closed-form time profiles and concrete plasma evolutions (Phase 4).
...
- Power-law decays T(t) = T_ref (t/t_ref)^-b and
  n_e(t) = n_ref (t/t_ref)^-a fit measured plasma histories well over
  the usual 0.5-10 us analysis window; see Aguilera & Aragon,
  Spectrochim. Acta Part B 59 (2004) 1861, and Cristoforetti et al.,
  Spectrochim. Acta Part B 59 (2004) 1907 (= thesis ref [171]).
...
class ExponentialDecay:
    ...
    Exponential decay profile:
        ...
        v(t) = initial_value * exp(-(t - start_time_s) / decay_time_s)

    for t >= start_time_s; clamped to initial_value before start_time_s
    (the parameter is taken as constant until its decay begins —
...
class PowerLawDecay:
    ...
    Aguilera & Aragon 2004; Cristoforetti et al. 2004 [thesis ref
    171]; module docstring):

        v(t) = reference_value * (t / reference_time_s)**(-exponent)

    Defined for t > 0 only: the power law diverges at t = 0, mirroring
    the breakdown singularity it approximates — evaluate at gate times,
    not at zero.
...
class ExpandingOnionEvolution(PlasmaEvolution):
    ...
    initial profile g1(r) = n_0^j (1 - k_2 r^2) (Eq. B-7, p. 268 =
    Eq. 5-37, p. 116),

        n_j(r, t) = (R_0/R(t))^3 * g1(r * R_0/R(t)),

    which is again parabolic with center density n_0^j (R_0/R)^3 and
    gradient k_2 (R_0/R)^2 — so each snapshot is generated by the
    Phase 3 `SphericalOnion.from_parabolic_profiles` factory
    unchanged. Note the invariant k_2(t) * R(t)^2 = k_2 R_0^2: a
    profile positive at t = 0 stays positive at all times.

    Documented simplification (development_rules.md): the *temperature*
    profile shape is assumed frozen in the self-similar coordinate
    r/R(t), T(r,t) = T_0(t) * (1 - k_1 (r R_0/R)^2), with the center
    history T_0(t) supplied by the user (e.g. a `PowerLawDecay`). The
    thesis instead evolves T through the energy-balance ODE (Eq. 5-35,
    p. 115 / App. C, Eq. C-21) — plug a numerical solution in through
    `CustomEvolution` when that fidelity is needed.
