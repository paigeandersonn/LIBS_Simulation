r"""Boltzmann level populations for LTE plasmas (Phase 2).

Physical context (Herrera 2008)
-------------------------------
Under local thermodynamic equilibrium a single excitation temperature
$T_{\mathrm{exc}}$ governs how the atoms/ions of a species $s$
distribute themselves over their bound energy levels. The thesis states
this as the Boltzmann relationship, Eq. 5-1, p. 98:

$$
n_k^{s} \;=\; n_{\mathrm{tot}}^{s}\,
\frac{g_k \exp\!\left(-E_k / (k_B T_{\mathrm{exc}})\right)}{U^{s}(T)}
$$

where $n_k^{s}$ is the number density in upper level $k$,
$n_{\mathrm{tot}}^{s}$ the total number density of species $s$ (one
element in one ionization stage), $g_k$ the level degeneracy, $E_k$
the level energy and $U^{s}(T)$ the partition function (symbols
pp. 19-23; $U(T)$ on p. 23).

The upper-level density $n_k^{s}$ computed here is exactly the
population that feeds the integrated line intensity of Eq. 5-8,
pp. 103-104 ($I_{ki} = n_k^{s} A_{ki}$), implemented in `emission`.
The linearized form of the same physics is the Boltzmann-plot
equation, Eq. 5-20, p. 109, used by CF-LIBS to invert for $T$ — a
Phase 5 concern, cited here for traceability only.

Units and conventions
---------------------
- Level energies arrive in eV (as stored on `Transition`) and are
  converted to Joules via `core.constants.EV` before dividing by
  $k_B T$ (SI throughout, development_rules.md).
- Number densities are m$^{-3}$; temperatures are Kelvin.
- "Species" means one element in one ionization stage (CF-LIBS usage,
  p. 103): pass the neutral-stage density with a stage-I transition,
  the singly-ionized density with a stage-II transition, etc.
  (densities come from the Saha balance in `saha`).

Numerical assumptions and limitations
-------------------------------------
- Functions are pure and vectorized; no global state.
- exp underflow for $E_k \gg k_B T$ flushes to 0.0 (correct physical
  limit).
- `boltzmann_population_fraction` uses an externally supplied $U(T)$
  (`partition`); it is exact relative to the true total density only
  in so far as $U(T)$ accounts for all bound levels (Herrera p. 260).
- `level_population_fractions` self-normalizes over the supplied
  level list, so the fractions sum to 1.0 by construction (Phase 2
  validation criterion) — but they refer to that truncated level set,
  not to a hypothetical complete one.
...
def boltzmann_population_fraction(
    ...
    $$
    \frac{n_k^{s}}{n_{\mathrm{tot}}^{s}} \;=\;
    \frac{g_k \exp\!\left(-E_k / (k_B T)\right)}{U^{s}(T)}
    $$
    ...
    energy_ev : array_like of float
        Level energy $E_k$ above the species ground state in eV
        (>= 0). Converted to Joules internally via
        `core.constants.EV`.
    ...
    temperature_K : array_like of float
        Excitation temperature $T_{\mathrm{exc}}$ in Kelvin (> 0).
        Under LTE this is the single plasma temperature of
        `PlasmaState` ($T_e = T_{\mathrm{exc}} = T_{\mathrm{ion}}$;
        `state` docs).

    Returns
    -------
    float or ndarray
        Dimensionless population fraction(s); lies in $[0, 1]$
        whenever $U(T)$ is consistent with (i.e. at least as large as
        the $g_k$ Boltzmann term of) the supplied level. Inputs
        broadcast under normal NumPy rules (e.g. arrays of levels
        against a scalar temperature); all-scalar input returns a
        float.

    Notes
    -----
    The fraction is exact only if $U^{s}(T)$ sums over all bound
    states (Herrera p. 260). Fractions over a *truncated* level list
    that must sum to exactly 1 are provided by
...
def upper_level_density(
    ...
    r"""
    LTE number density of the upper level of a transition, $n_k^{s}$
    (m$^{-3}$).
    ...
    This $n_k^{s}$ is the population that multiplies $A_{ki}$ in the
    integrated line intensity, Eq. 5-8, pp. 103-104 (`emission`).

    Parameters
    ----------
    transition : Transition
        Atomic transition; supplies `g_upper` and `energy_upper_ev`
        (eV, converted to J internally).
    species_density_m3 : array_like of float
        $n_{\mathrm{tot}}^{s}$ in m$^{-3}$ (>= 0): density of the
        element *in the transition's ionization stage* (neutral
        density for stage-I lines, ion density for stage-II), as
        returned by the Saha balance (`saha`, Eqs. D-8/D-10,
        pp. 275-276).
    partition_function : array_like of float
        $U^{s}(T)$ for the same species and temperature(s),
        dimensionless.
    temperature_K : array_like of float
...
def level_population_fractions(
    ...
    (`partition.partition_function_from_levels`, the p. 260
    definition), so that the returned fractions sum to 1.0 by
    construction — the Phase 2 validation criterion "population
    fractions must sum to 1.0" (validation_strategy.md).
    ...
    temperature_K : float
        Temperature in Kelvin (> 0); scalar, one distribution at a
        time.

    Returns
    -------
    ndarray, shape (n_levels,)
        Population fractions $n_k / n_{\mathrm{tot}}$, summing to 1.0
        (within one floating-point rounding of the final
        normalization).
